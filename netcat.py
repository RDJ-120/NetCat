...
scan.add_argument("-a", "--all", help="Scan All Network", action="store_true")
...
if args.cmd == "chat":
        ...
        def start():
            ...
            while True:
                ...
                if password:
                    ...
                    conn.sendall(passed_length)
                    conn.sendall(pass_msg)

                    pass_length = conn.recv(PASS).decode("utf-8").strip()
                    pass_length = int(pass_length)
                    sent_password = recv_all(conn, pass_length).decode("utf-8")

                    if sent_password == password:
                        t = threading.Thread(target=handle, args=(conn, addr, n))
                        t.start()
                    else:
                        conn.close()
                else:
    ...
    if args.client:
        ...
        name_length += b' ' * (NAME - len(name_length))
        client.sendall(name_length)
        client.sendall(name_enc)

        if args.putpassword:
            length = client.recv(PASS).decode("utf-8").strip()
            length = int(length)
            message = recv_all(client, length).decode("utf-8")
...
def sendfile(filepath, ADDR):
    ...
    name_length = str(len(name)).encode("utf-8")
    name_length += b' ' * (32 - len(name_length))
...
if args.cmd == "filesend":
    ...
    if args.sender:
        ...
        if filepath.is_dir():
            shutil.make_archive(name, 'zip', filepath)
            name = f"{name}.zip"
            full = parent.joinpath(name)
            sendfile(full, ADDR)
            full.unlink()
    ...
    if args.receiver:
        ...
        length = int(length)
        name = recv_all(client, length).decode("utf-8")

        size_bytes = recv_all(client, 128)
        size = int(size_bytes.decode("utf-8").strip())
